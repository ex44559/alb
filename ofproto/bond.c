...
bond_mode_from_string(enum bond_mode *balance, const char *s)
{
    if (!strcmp(s, bond_mode_to_string(BM_TCP))) {
        ...
        *balance = BM_AB;
    } else if (!strcmp(s, bond_mode_to_string(BM_ALB))) {
		*balance = BM_ALB;
	} else {
        return false;
    }
...
static bool
bond_is_ALB(const struct bond *bond) OVS_REQ_RDLOCK(rwlock)
{
    return bond->rebalance_interval
        && (bond->balance == BM_ALB);
}
...
static void
insert_bal_ALB(struct ovs_list *bals, struct bond_slave *slave)
{
...
ALB_rebalance(struct bond *bond)
{
    ...
    HMAP_FOR_EACH (slave, hmap_node, &bond->slaves) {
        if (slave->enabled) {
			...
			slave->score = (slave->speed / (slave->tx_bytes + 1));
...
/*
Maybe needed to be removed

static unsigned int
bond_hash_alb(const struct eth_addr mac, uint16_t vlan, uint32_t basis)
{
...
static struct bond_entry *
lookup_bond_entry_ALB(const struct bond *bond, const struct flow *flow,
                  uint16_t vlan)
{
    return &bond->hash[bond_hash_alb(flow->dl_dst, vlan, bond->basis) & BOND_MASK];
}
*/
